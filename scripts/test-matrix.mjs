...
async function req(path, opts = {}) {
  ...
  if (setCookie && /ccc_account/.test(setCookie)) customerCookie = setCookie.split(';')[0];
...
/*
 * The menu is approval-gated, so most "public" checks below need an approved
 * account. Create one, approve it as admin, and keep its cookie.
 */
console.log('\nACCOUNT GATING\n');

const testEmail = 'qa-' + Date.now() + '@example.com';
const signup = await jsonReq('/api/account/signup', 'POST', {
  name: 'QA Customer', email: testEmail, phone: '5105551234', password: 'qa-password-123'
});
record('SIGNUP', signup.status === 201);

const anonHome = await req('/', { headers: { cookie: 'ccc_age_ok=1' } });
record('MENU HIDDEN FROM ANONYMOUS', anonHome.text.includes('Approved accounts only'));
record('NO INVENTORY LEAKED TO ANONYMOUS',
  !/\\"qty\\"|\\"pricing\\"|\\"sku\\"/.test(anonHome.text));
record('SITEMAP HIDES STRAIN URLS', !(await req('/sitemap.xml')).text.includes('/strains/'));

const pendingSignin = await jsonReq('/api/account/signin', 'POST', {
  email: testEmail, password: 'qa-password-123'
});
record('SIGNIN WHILE PENDING', pendingSignin.status === 200 && pendingSignin.json?.status === 'PENDING');
record('PENDING STILL CANNOT SEE MENU',
  (await req('/', { headers: { cookie: 'ccc_age_ok=1; ' + customerCookie } })).text.includes('Awaiting approval'));

// Approve as admin.
const adminLogin = await jsonReq('/api/auth', 'POST', { password: PASSWORD });
record('ADMIN LOGIN (for approval)', adminLogin.status === 200);
const custList = await req('/api/customers');
record('CUSTOMER LIST HIDES PASSWORD HASH', !/passwordHash|password_hash/.test(custList.text));
const qa = custList.json?.find?.((c) => c.email === testEmail);
record('CUSTOMER APPEARS IN ADMIN', !!qa);
if (qa) {
  const approved = await jsonReq('/api/customers/' + qa.id, 'PATCH', { status: 'APPROVED' });
  record('ADMIN APPROVES ACCOUNT', approved.status === 200 && approved.json?.status === 'APPROVED');
  record('REJECTS INVALID STATUS',
    (await jsonReq('/api/customers/' + qa.id, 'PATCH', { status: 'SUPERADMIN' })).status === 422);
}

// Drop the admin session again so the AUTH section below genuinely tests the
// logged-out state rather than inheriting this one.
await req('/api/auth', { method: 'DELETE' });
cookie = '';

const asCustomer = { cookie: 'ccc_age_ok=1; ' + customerCookie };
record('APPROVED ACCOUNT SEES MENU',
  !(await req('/', { headers: asCustomer })).text.includes('Approved accounts only'));
...
for (const [name, path, expect] of [
  ...
  const res = await req(path, { headers: asCustomer });
...
const approvedHome = await req('/', { headers: asCustomer });
const firstSlug = (approvedHome.text.match(/\/strains\/([a-z0-9-]+)"/) || [])[1];
if (firstSlug) {
  const detail = await req('/strains/' + firstSlug, { headers: asCustomer });
  record('STRAIN DETAIL', detail.status === 200, '/strains/' + firstSlug);
} else {
  record('STRAIN DETAIL', false, 'no published strain found in sitemap');
}

const home = await req('/', { headers: asCustomer });
...
if (slug) {
  ...
  const pub = await req('/strains/' + slug, { headers: asCustomer });
  ...
  record('UNPUBLISHED IS 404 PUBLICLY',
    (await req('/strains/' + slug, { headers: asCustomer })).status === 404);
