...
const TITLES = {
  ...
  customers: ['Accounts', 'Menu access requests']
};

export default function AdminClient({ initialStrains, initialInquiries, initialCustomers = [], dataDriver }) {
  ...
  /**
   * Approve, reject or suspend an account.
   *
   * Applied optimistically and rolled back on failure, matching the rest of the
   * panel — the operator should never be unsure whether a click registered.
   */
    ...
    <div className="cc-admin-shell">
      ...
      <main style={{ minWidth: 0, padding: '26px clamp(18px,2.6vw,34px) 90px' }}>
        ...
        {tab === 'customers' ? (
          <section style={{ animation: 'ccIn .3s ease', marginTop: 26 }}>
            {customers.length === 0 ? (
              <p style={{ font: '400 12px/1.7 ' + MONO, color: C.dim }}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                {customers.map((c) => {
                  const tone =
                    c.status === 'APPROVED' ? '#63b487'
                      : c.status === 'PENDING' ? '#e0a04a'
                        : '#9aa79c';
                  return (
                    <div
                      key={c.id}
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                        <div style={{ minWidth: 0 }}>
                          <div style={{ font: '600 13px/1.3 ' + SANS }}>{c.name}</div>
                          <div style={{ font: '400 11.5px/1.6 ' + MONO, color: C.dim, wordBreak: 'break-word' }}>
                            {c.email}
                          </div>
                          <a
                            href={'tel:' + String(c.phone || '').replace(/[^0-9+]/g, '')}
                            style={{ font: '400 11.5px/1.6 ' + MONO, color: C.text }}
                          >
                            {c.phone}
                          </a>
                          {c.company ? (
                            <div style={{ font: '400 11px/1.6 ' + SANS, color: C.faint }}>{c.company}</div>
                          ) : null}
                        </div>
                        <div style={{ textAlign: 'right' }}>
                          ...
                          <div style={{ font: '400 10px/1.6 ' + MONO, color: C.faint, marginTop: 6 }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        {[
                          ...
                          <button
                            key={value}
                            type="button"
                            onClick={() => setCustomerStatus(c.id, value)}
                            aria-pressed={c.status === value}
                            style={{
                              flex: '1 1 auto',
                              minHeight: 44,
                              padding: '0 14px',
                              borderRadius: 2,
                              border: '1px solid ' + (c.status === value ? tone : 'rgba(234,240,234,.16)'),
                              background: c.status === value ? 'rgba(234,240,234,.05)' : 'transparent',
                              color: c.status === value ? tone : C.dim,
                              font: '600 10px/1 ' + SANS,
                              letterSpacing: '.16em',
                              textTransform: 'uppercase',
                              cursor: 'pointer'
