...
export default async function StrainPage({ params }) {
  ...
  // Gating the menu grid alone would be cosmetic: the detail URLs are guessable
  // and appear in the sitemap. Unapproved visitors are sent to sign in.
  const { customer, approved } = await currentCustomer();
  if (!approved) redirect(customer ? '/account' : '/signin');
  const strain = await getStrain(slug);
