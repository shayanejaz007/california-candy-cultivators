...
// Every select lists columns explicitly rather than '*', so password_hash
// cannot leak into a response by accident if the table gains columns later.
const CUSTOMER_COLS =
  'id, email, name, phone, company, status, admin_notes, created_at, updated_at, approved_at';
...
export async function allCustomers() {
  ...
  if (error) throw error;
  return (data || []).map(customerToApp);
}

export async function getCustomerById(id) {
  const { data, error } = await conn()
    .from('customers').select(CUSTOMER_COLS).eq('id', id).maybeSingle();
  if (error) throw error;
  return data ? customerToApp(data) : null;
}

/** Internal: the only place password_hash is ever selected. */
export async function getCustomerForAuth(email) {
  const { data, error } = await conn()
    .from('customers')
    .select(CUSTOMER_COLS + ', password_hash')
    .eq('email', normaliseEmail(email))
    .maybeSingle();
  if (error) throw error;
  if (!data) return null;
  return { ...customerToApp(data), passwordHash: data.password_hash };
}
...
export async function createCustomer(input) {
  ...
  const { data, error } = await conn()
    .from('customers').insert(row).select(CUSTOMER_COLS).single();

  // 23505 is unique_violation — the email is already registered.
  if (error?.code === '23505') return { duplicate: true };
  if (error) throw error;
  return { customer: customerToApp(data) };
}
...
export async function updateCustomer(id, patch) {
  ...
  if (typeof patch.status === 'string') {
    ...
    row.approved_at = patch.status === 'APPROVED' ? new Date().toISOString() : null;
  }
  if (typeof patch.adminNotes === 'string') row.admin_notes = patch.adminNotes.slice(0, 2000);

  const { data, error } = await conn()
    .from('customers').update(row).eq('id', id).select(CUSTOMER_COLS).maybeSingle();
  if (error) throw error;
  return data ? customerToApp(data) : null;
}

export async function deleteCustomer(id) {
  const { error } = await conn().from('customers').delete().eq('id', id);
  if (error) throw error;
  return true;
}
