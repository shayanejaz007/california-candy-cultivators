...
const FIELDS = [
  ...
  'featured', 'sort', 'sku', 'batchName', 'batchNo', 'harvest', 'release', 'teaser',
...
/** Emails are matched case-insensitively; the original casing is preserved. */
const normaliseEmail = (v) => String(v || '').trim().toLowerCase();

function customerToApp(c) {
  // The hash never leaves the data layer.
...
export async function allCustomers() {
  const d = await load();
  return structuredClone(d.customers || [])
...
export async function getCustomerById(id) {
  const d = await load();
  const found = (d.customers || []).find((c) => c.id === id);
  return found ? customerToApp(structuredClone(found)) : null;
}

/** Internal: returns the row WITH its hash, for sign-in only. */
export async function getCustomerForAuth(email) {
  const d = await load();
  const found = (d.customers || []).find((c) => c.email === normaliseEmail(email));
...
export function createCustomer(input) {
  return transaction((d) => {
    ...
    if (d.customers.some((c) => c.email === email)) {
      // Signalled rather than thrown so the route can answer without leaking
    ...
    return { customer: customerToApp(structuredClone(row)) };
  });
}
...
export function updateCustomer(id, patch) {
  return transaction((d) => {
    ...
    if (typeof patch.status === 'string' && ACCOUNT_STATUSES.has(patch.status)) {
      if (patch.status !== c.status) {
        c.status = patch.status;
        c.approvedAt = patch.status === 'APPROVED' ? new Date().toISOString() : null;
      }
    }
    if (typeof patch.adminNotes === 'string') {
      c.adminNotes = patch.adminNotes.slice(0, 2000);
    }
    c.updatedAt = new Date().toISOString();
    return customerToApp(structuredClone(c));
  });
}
